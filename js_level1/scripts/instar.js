...
function profileGo(){
    return window.location.href = 'instar_profile.html'
}
//2. 우측... 메뉴 클릭 시 저장~신고 등 기능 실행 (o)-저장~신고제작필요
//3. 사진 한번 터치 시 반응 없음(x js)
...
//위 매개변수가 받은 정보를 확인하려면 매개변수에 변수명을 작성하고 console.log()로 확인할수 있다.
likeImg.addEventListener('click',function(e){
    //console.log(e);
    e.preventDefault();//태그의 동적기능 취소(a태그새로고침)
    likeOn();
})
//6. 댓글 아이콘 터치 시 댓글 입력창 실행(o)-제작필요, 실행화면에서 바깥쪽 영역 터치 댓글 입력창 닫힘(o)
//7. 공유 아이콘 터치 시 공유가능 계정들 표시된 창 실행(o)-제작필요, 바깥쪽 영역 터치시 닫힘(o)
//8. 즐겨찿기 아이콘 터치 시 즐겨찿기 저장(x-DB필요) 이 됨과 동시에 컬렉션 선택 화면 출력(o), 바깥쪽 영역 터치 시 닫힘
//9. 글 1줄, 날짜만 미리표시, 글 터치 시 전체 내용(나머지 글줄과 해시태그 포함) 펼치기(o)
...
const contentsTag = document.querySelector('.tag');
console.log(contentsTag);
contentsTag.addEventListener('click',contentsGo);
function contentsGo(){
    return window.location.href = 'instar_tag.html'
}
//별도의 동적기능 없이 단순한 링크만 있는 경우는 js없이 a태그에 링크걸기
//동적기능 예) 해외IP접속차단,동일IP접속차단,성인사이트접족인증,로그인에 따른 접속가능 페이지일 경우 로그인 유무 확인 등
//11. 날짜 터치 기능 없음 (x js)
//추가 페이지 목록) 프로필 페이지(insta_profile.html) 저장~신고 페이지 댓글입력창 공유가능 계정들 표시된 창 태그 사용 게시글(instar_tag.html)