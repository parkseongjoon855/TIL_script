...
let g ;//전역변수 생성 (값 선언 안된 상태 undefiend)
scopeTest();//호출(함수 실행)
...
function scopeTest(){//함수 호출 전 대기상태의 함수 생성구조
    let f = 25;//지역변수
    let g = 35;//지역변수
    console.log(d+f);//실행가능 5+25
    console.log(e+g);//실행가능 15+35
}
scopeTest();//호출(함수 실행)
console.log(f);//실행불가
console.log(g+d);//실행불가
console.log(d+e);//실행가능 5+15
...
console.log('------------------------')
    ...
    function storyFunc1(){
        ...
        console.log(`${person[0]}과${person[1]}이 ${src}을이용해서 집을 찾아갔습니다.`);
    }
    function storyFunc2(){
        let person = ['헨젤','그레텔']
        let src = '빵부스러기';
    ...
    //function coffeeFunc1(data1, data2){ 실행함수+매개변수data1활용 } 
...
///더하기 알고리즘
//함수 내부{}는 대기장소롤 호출하지 않는한 절대 실행되지 않는다
...
//대입연산자(=)와 화살표함수(=>)를 붙여서 사용할 수 없기때문에 매개변수와 함수선언표시 순서를 바꾸서 =()=> 로 사용한다
//일반함수 경우
function bab(num){
    return '밥' +num+'공기주세요';
}
...
//반찬 주세요
//국 주세요
const  bab = (data) => {return data+ '나왔습니다'};
console.log(bab3('밥'));
console.log(bab3('반찬'));
console.log(bab3('국'));